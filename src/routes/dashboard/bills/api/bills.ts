...
export type BillMutationFields = Pick<
  BillResponse,
  "elec_readings" | "water_readings" | "month" | "year" | "shop"
>;
export type BillUpdateFields = Pick<
  BillResponse,
  "elec_readings" | "water_readings" | "month" | "year" | "shop" | "id"
>;
...
export async function getOneMonthlyBill(
  pb: PocketBaseClient,
...
export async function updateBill(pb: PocketBaseClient, bill: BillUpdateFields) {
  try {
    ...
    if (error instanceof ClientResponseError) {
      console.log(
        "========= ClientResponseError UPDATING BILL ========== ",
        error.toJSON(),
      );
    }
    throw error;
