...
import type { LiquorstoreCategoriesUpdate } from "@/lib/pb/database";
...
interface UpdateCategoryProps {
  id: string;
  item: LiquorstoreCategoriesUpdate;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
}

const collectionName: CollectionName = "liquorstore_categories";
export function UpdateCategory({ id, item, setOpen }: UpdateCategoryProps) {
  const { pb, viewer } = usePocketbase();
  const { input, handleChange, setInput } =
    useFormHook<LiquorstoreCategoriesUpdate>({
      initialValues: {
        name: item.name,
      },
    });
  const mutation = useMutation({
    mutationFn: (data: LiquorstoreCategoriesUpdate) => {
    ...
    onSuccess: (data) => {
      if (data) {
        ...
        if (data.error) {
          sonnerToast({
            type: "error",
            title: "Something went wrong while updating category entry",
            options: {
    ...
    >
      ...
      <DialogContent className="sm:max-w-[80%] min-w-[80%] md:min-w-[60%] lg:min-w-[40%] w-fit  overflow-auto">
        ...
        <div className="w-full h-full ">
          <UpdateCategory id={id} item={item} setOpen={setOpen} />
        </div>
